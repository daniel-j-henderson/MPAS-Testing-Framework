...
def retrieveFileFromSL(name, dest, env):
	...
	if env.get('pathSL'):
		...
		print("No SL in this environment, must download necessary files from web")
		retrieveFileFromWebSL(name, dest, env)
		return
...
def writeReportTex(f, results):
	...
	for r in results:
		print(r.attributes)
		if not r.getAttribute('success'):
			continue
		f.write("\\subsection{"+r.getAttribute('name')+"}\n")
